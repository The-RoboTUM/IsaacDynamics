...
import isaaclab_tasks.manager_based.custom.pendulum_grav_simple.mdp as mdp

sys.path.append("/home/andrew/Documents/IsaacLab")
...
class ActionsCfg:
    ...
    joint_effort = mdp.JointEffortActionCfg(asset_name="robot", joint_names=["base_to_pendulum"], scale=100.0)
...
class EventCfg:
    ...
    reset_pendulum_position = EventTerm(
        ...
        mode="reset",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=["base_to_pendulum"]),
...
class RewardsCfg:
    ...
    # (3) Primary task: keep pendulum upright
    pole_pos = RewTerm(
    ...
    # )
    # (5) Shaping tasks: lower pendulum angular velocity
    pole_vel = RewTerm(
        func=mdp.joint_vel_l1,
        weight=-0.005,
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=["base_to_pendulum"])},
    )
